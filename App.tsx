...
import { ThemeColor, SalesTransaction, PaymentTransaction, Customer, PurchaseTransaction, ExpenseTransaction, StockItem, StockTransaction, Account, GlobalTransaction, AppSettings, Product, Reminder, ExpenseCategory } from './types';
...
export default function App() {
  ...
  const TOTAL_COLLECTIONS = 12; // sales, payments, customers, purchases, expenses, stocks, stockTransactions, accounts, globalTransactions, products, reminders, expenseCategories
  ...
  const [expenseCategories, setExpenseCategories] = useState<WithDocId<ExpenseCategory>[]>([]);
  ...
  useEffect(() => {
    ...
    const expenseCategoriesUnsub = subscribeToCollection<WithDocId<ExpenseCategory>>('expenseCategories', (data) => {
      setExpenseCategories(data);
      markLoaded('expenseCategories');
    }, 'createdAt');
    if (expenseCategoriesUnsub) {
      unsubscribes.push(expenseCategoriesUnsub);
    } else {
      markLoaded('expenseCategories');
    }
  ...
  const updateGlobalTransactionByExpenseId = async (expenseId: number, newAmount: string, description?: string, newDate?: string) => {
    const tx = globalTransactions.find(t => (t as any).expenseId === expenseId) as WithDocId<GlobalTransaction> | undefined;
    if (!tx) return;

    const updatedTx = { ...tx, amount: newAmount, ...(description && { description }), ...(newDate && { date: newDate }) };
  ...
  // Update globalTransaction by paymentId (for syncing when payment is edited)
  const updateGlobalTransactionByPaymentId = async (paymentId: number, newAmount: string, description?: string, newDate?: string) => {
    const tx = globalTransactions.find(t => (t as any).paymentId === paymentId) as WithDocId<GlobalTransaction> | undefined;
    if (!tx) return;

    // Calculate the balance difference
    const oldAmount = parseFloat(tx.amount.replace(/,/g, '')) || 0;
    const newAmountNum = parseFloat(newAmount.replace(/,/g, '')) || 0;
    ...
    if (firebaseReady && tx._docId) {
      ...
      setGlobalTransactions(prev => prev.map(t => (t as any).paymentId === paymentId ? updatedTx : t));
    }

    // Adjust account balance for the difference
    // For Credit transactions (payments), positive difference means more money credited
    if (difference !== 0) {
      const balanceChange = tx.type === 'Credit' ? difference : -difference;
  ...
  // Expense Categories CRUD - Firebase synced
  const addExpenseCategory = async (category: ExpenseCategory) => {
    if (firebaseReady) {
      const customId = `category_${sanitizeForDocId(category.name)}`;
      await addDocument('expenseCategories', category, customId);
    } else {
  ...
  const deleteExpenseCategory = async (id: number) => {
    const category = expenseCategories.find(c => c.id === id) as WithDocId<ExpenseCategory> | undefined;
    if (firebaseReady && category?._docId) {
      await deleteDocument('expenseCategories', category._docId);
    } else {
  ...
  const dataValue = {
    ...
    updateGlobalTransactionByPaymentId,
    settings,
    updateSettings,
    sales,
    addSale,
