...
function CustomerDetails({ customer, onBack }: { customer: any, onBack: () => void }) {
    ...
    const { sales, payments, addSale, updateSale, deleteSale, addPayment, updatePayment, deletePayment, addGlobalTransaction, updateGlobalTransactionByPaymentId, updateCustomer, settings } = useContext(DataContext);
    const themeConfig = THEMES[theme];
    ...
    const handleSaveOpeningBalanceBoth = (amount: string, date: string, phone: string) => {
        const updatedCustomer = { ...localCustomer, openingBalance: amount, openingBalanceDate: date, phone };
        updateCustomer(updatedCustomer);
        setLocalCustomer(updatedCustomer);
    ...
    const handleSaveSale = (data: any) => {
        ...
        const newSale = {
            ...
            type: 'Sale' as const,
            note: data.notes,
            ...(editingTransaction?._docId && { _docId: editingTransaction._docId })
        };
    ...
    const handleSavePayment = (data: any) => {
        ...
        if (editingTransaction) {
            ...
            // Sync the linked globalTransaction if it exists
...
// Modal for editing both opening balance amount, date, and phone
function EditOpeningBalanceModal({
    ...
    phone: string,
    onSave: (amount: string, date: string, phone: string) => void,
    onClose: () => void
}) {
    const { isDarkMode, theme } = useContext(ThemeContext);
    const themeConfig = THEMES[theme];
    ...
    const handleSave = () => {
        onSave(amount, formatDate(date), phone);
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className={`relative w-full max-w-sm rounded-xl border shadow-2xl flex flex-col ${isDarkMode ? 'bg-[#09090b] border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <div className="flex items-center justify-between p-4 border-b dark:border-zinc-800">
                    <h2 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>Edit Customer Details</h2>
                    <button onClick={onClose} className={`p-1 rounded-full transition-colors ${isDarkMode ? 'hover:bg-zinc-800 text-zinc-400' : 'hover:bg-zinc-100 text-zinc-500'}`}>
                        <X size={18} />
                    </button>
                </div>
                <div className="p-5 space-y-4">
                    <div className="space-y-1.5">
                        <label className={`text-xs font-medium ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>Phone Number</label>
                        <input
                            type="tel"
                            value={phone}
