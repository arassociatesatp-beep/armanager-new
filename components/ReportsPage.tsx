...
export default function ReportsPage() {
    ...
    const filteredPurchases = useMemo(() => {
        return purchases.filter(p => {
            ...
            if (activeReport === 'Sub Category Report') {
                // If filtering by purchase category (Direct/AR & Co/GL/GV)
    ...
    const subCategoryReportData = useMemo(() => {
        ...
        purchases.forEach(p => {
            ...
            if (selectedCategory !== 'All' && p.subCategory !== selectedCategory) return;
            if (selectedSubCatProduct !== 'All' && p.item !== selectedSubCatProduct) return;
            if (searchQuery) {
                const q = searchQuery.toLowerCase();
                if (!p.vendor.toLowerCase().includes(q) && !p.item.toLowerCase().includes(q)) return;
            }
            ...
            // Filter out purchases after end date
            ...
            if (from && pDate < from) {
                ...
                if (cat === 'Direct') carryDirect += qty;
                else if (cat === 'AR & Co') carryArCo += qty;
                else if (cat === 'GL') carryGl += qty;
                else if (cat === 'GV') carryGv += qty;

                carryBilled += billedQty;
                carryTotal += qty;
            } else {
                // Within date range
                ...
                if (!groups[key]) {
                    groups[key] = {
                        ...
                        dateObj: pDate,
                        product: p.item,
                        direct: 0, arCo: 0, gl: 0, gv: 0, billed: 0, total: 0
                    };
                }

                if (cat === 'Direct') groups[key].direct += qty;
                else if (cat === 'AR & Co') groups[key].arCo += qty;
        ...
        const carryUnbilled = (carryGl + carryGv) - carryBilled;
        let runningUnbilled = carryUnbilled;

        const withBilling = chronologicalRows.map(row => {
            const inflow = row.gl + row.gv;
            const totalBilled = row.billed;
            const totalUnbilled = runningUnbilled + (inflow - totalBilled);
            runningUnbilled = totalUnbilled;
        ...
        result.push(...withBilling);
        return result;
    }, [purchases, dateRange, selectedCategory, selectedSubCatProduct, searchQuery, settings.bagsPerTon, selectedMetric]);

    const subCategoryColumnTotals = useMemo(() => {
        const totals = { direct: 0, arCo: 0, gv: 0, gl: 0, total: 0, totalBilled: 0, totalUnbilled: 0 };
        subCategoryReportData.forEach((row) => {
            totals.direct += row.direct;
            totals.arCo += row.arCo;
            totals.gv += row.gv;
            totals.gl += row.gl;
    ...
    const handleDownload = async () => {
        ...
        const baseTableStartY = 50; // extra spacing below header/meta (adjusted for AR Associates header)
        ...
        // Add "AR Associates" blue header banner to PDF
        const addCompanyHeader = () => {
            // Blue header rectangle - RGB for #1e3a8a
            doc.setFillColor(30, 58, 138);
            doc.rect(0, 0, 210, 18, 'F'); // Full width, 18mm height

            // White text for company name
            doc.setTextColor(255, 255, 255);
            doc.setFontSize(16);
            doc.setFont(undefined as any, 'bold');
            doc.text('AR Associates', 14, 12);
        ...
        if (activeReport === 'Notes Report') {
            addHeader("Notes Report", 28);
            doc.setFontSize(12);
            doc.text(`Exported on: ${new Date().toLocaleDateString()}`, 14, 40);
            doc.setFontSize(10);
        ...
        if (activeReport === 'Customer Category Report') {
            addHeader(`Customer List: ${selectedCategory}`, 28);
        ...
        // --- Customer Ledger ---
            ...
            if (selectedLedgerCustomer && ledgerDetails) {
                ...
                // Header - Simple format with company banner
                addCompanyHeader();

                doc.setFontSize(18);
                doc.setFont(undefined as any, 'bold');
                doc.text(`Customer Ledger: ${customer.name}`, 14, 30);

                doc.setFontSize(10);
                doc.setFont(undefined as any, 'normal');
                doc.text(`Report Date: ${new Date().toLocaleDateString('en-GB')}`, 14, 38);
                doc.text(`Phone: ${customer.phone}`, 14, 45);
                doc.text(`Opening Balance: Rs. ${ledgerDetails.openingBalance.toLocaleString()} (as on ${formatDisplayDate(ledgerDetails.openingBalanceDate)})`, 14, 52);
        ...
        // --- Sub Category Report ---
            ...
            autoTable(doc, {
                startY: baseTableStartY,
                head: [['Date', 'Product', 'Direct', 'AR & Co', 'G.V', 'GL', 'Total Billed', 'Total Unbilled', 'TOTAL']],
                ...
                foot: [[
                    ...
                    subCategoryColumnTotals.arCo.toFixed(2),
                ...
                styles: { fontSize: 8 },
                didParseCell: (data) => {
                    if (data.section === 'body' && subCategoryReportData[data.row.index]?.isCarryForward) {
        ...
        // --- Daily Report ---
            ...
            addHeader("Daily Transaction Report", 28);
            let currentY = 45;
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left text-sm">
                                    <thead><tr className={`border-b text-xs uppercase tracking-wider ${isDarkMode ? 'border-zinc-800 text-zinc-500 bg-zinc-900/50' : 'border-zinc-100 text-zinc-500 bg-zinc-50'}`}>
                                        ...
                                        <th className="py-3 px-4 font-semibold text-right">AR & Co</th>
                                    ...
                                    <tbody className={`divide-y ${isDarkMode ? 'divide-zinc-800' : 'divide-zinc-100'}`}>
                                        ...
                                        {subCategoryReportData.length > 0 && (
                                            <tr className={`font-semibold ${isDarkMode ? 'bg-zinc-900/40 text-zinc-200' : 'bg-zinc-50 text-zinc-800'}`}>
                                                ...
                                                <td className="py-3 px-4 text-right">{subCategoryColumnTotals.arCo.toFixed(2)}</td>
