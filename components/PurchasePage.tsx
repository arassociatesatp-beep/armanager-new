...
const categories = ['Direct', 'AR & Co', 'GL', 'GV'];

// Products are now fetched from the DataContext instead of being hardcoded

const PURCHASE_TYPES = ['Direct', 'AR & Co', 'GL', 'GV'];
const QUANTITY_UNITS = ['bags', 'tons', 'kg', 'm3', 'buckets', 'pieces'];

export default function PurchasePage() {
    const { isDarkMode, theme } = useContext(ThemeContext);
    const { settings, purchases, addPurchase, updatePurchase, deletePurchase, expenses, addExpense, updateExpense, deleteExpense, addGlobalTransaction, updateGlobalTransactionByExpenseId, deleteGlobalTransactionByExpenseId, updateAccountBalance, accounts, products, dataLoading, expenseCategories, addExpenseCategory } = useContext(DataContext);
    ...
    // Expense Filters - synced via Firebase collection
    const [selectedExpenseCategory, setSelectedExpenseCategory] = useState('All');

    // Get expense categories from Firebase collection, with defaults
    const expenseFilterCategories = useMemo(() => {
        const categoriesFromDb = expenseCategories.map(c => c.name);
        // Add 'All' at the start and merge with defaults (avoiding duplicates)
        const defaultCategories = initialExpenseCategories.filter(c => c !== 'All');
        const allCategories = [...new Set([...defaultCategories, ...categoriesFromDb])];
        return ['All', ...allCategories];
    }, [expenseCategories]);

    // Function to add a new expense category to Firebase collection
    const handleAddExpenseCategory = (categoryName: string) => {
        if (categoryName && !expenseFilterCategories.includes(categoryName)) {
            const newCategory = {
                id: Date.now(),
                name: categoryName
            };
    ...
    const handleSaveExpense = (data: any) => {
        // If a new category was created (custom), add it to Firebase collection
        if (data.item && !expenseFilterCategories.includes(data.item)) {
            handleAddExpenseCategory(data.item);
        }
        ...
        if (editingItem) {
            ...
            // Update the linked globalTransaction to reflect the new amount and date
            const dateStr = data.date ? formatDate(data.date) : formatDate(new Date());
            updateGlobalTransactionByExpenseId(editingItem.id, data.amount, `Expense: ${data.item} (${data.vendor})`, dateStr);
                    ...
                    <div className={`px-4 pb-4 flex items-center gap-3 animate-in fade-in slide-in-from-top-1 duration-200`}>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="relative" ref={fromDateRef}>
                                ...
                                {isFromDateOpen && (
                                    <>
                                        <div className="fixed inset-0 z-40 bg-black/50 sm:hidden" onClick={(e) => { e.stopPropagation(); setIsFromDateOpen(false); }} />
                                        <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none sm:pointer-events-auto sm:absolute sm:inset-auto sm:top-full sm:left-0 sm:mt-2">
                            ...
                            <div className="relative" ref={toDateRef}>
                                ...
                                {isToDateOpen && (
                                    <>
                                        <div className="fixed inset-0 z-40 bg-black/50 sm:hidden" onClick={(e) => { e.stopPropagation(); setIsToDateOpen(false); }} />
                                        <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none sm:pointer-events-auto sm:absolute sm:inset-auto sm:top-full sm:right-0 sm:mt-2">
                                            <div className="pointer-events-auto relative">
                                                <CustomCalendar
                                                    value={toDate}
            ...
            <div className={`relative w-full max-w-md rounded-xl border shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto scrollbar-hide animate-in zoom-in-95 duration-200 ${isDarkMode ? 'bg-[#09090b] border-zinc-800' : 'bg-white border-zinc-200'}`}>
                ...
                <div className="p-5 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div><label className={labelClass}>Type</label><CustomDropdown options={PURCHASE_TYPES} value={subCategory} onChange={setSubCategory} placeholder="Direct/AR & Co/GL/GV" className="w-full text-xs" /></div>
