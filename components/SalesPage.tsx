...
export default function SalesPage() {
    ...
    const { sales, payments, customers, addSale, updateSale, deleteSale, addPayment, updatePayment, deletePayment, addGlobalTransaction, updateGlobalTransactionByPaymentId, dataLoading, settings } = useContext(DataContext);
        ...
        if (editingItem) {
            ...
            if (editingItem.accountId) {
                updateGlobalTransactionByPaymentId(
                    newPayment.id,
                    paymentData.amount,
                    `Payment from ${paymentData.customer}`,
                    dateStr
                );
            }
        } else {
...
export function RecordPaymentModal({ onClose, onRecordPayment, initialData, preselectedCustomer }: { onClose: () => void, onRecordPayment: (data: any) => void, initialData?: any, preselectedCustomer?: string }) {
    ...
    useEffect(() => {
        if (initialData) {
            ...
            setNotes(initialData.note || '');
            // Pre-populate the account field when editing
            if (initialData.accountId) {
                setSelectedAccountId(String(initialData.accountId));
            }
            if (initialData.date) {
                const [d, m, y] = initialData.date.split('-').map(Number);
                setSelectedDate(new Date(y, m - 1, d));
