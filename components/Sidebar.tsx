    ...
    return (
        ...
        `}
            ...
            <div className={`flex items-center mb-8 gap-3 transition-all duration-300 ${isCollapsed ? 'px-0 justify-center' : 'px-6'}`}>
                <div className={`relative flex items-center justify-center shrink-0 w-8 h-8 rounded-lg font-bold text-xs ${isDarkMode ? 'bg-white text-black' : 'bg-zinc-950 text-white'}`}>
                    AR
                </div>
                {!isCollapsed && (
                    <div className="flex flex-col">
                        <span className={`text-sm font-bold tracking-tight leading-none whitespace-nowrap ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>AR ASSOCIATES</span>
                        <span className={`text-[9px] font-medium tracking-wide uppercase ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>Cement Business</span>
                    </div>
                )}
            </div>
